...
int main()
{
	...
	// cv::imwrite("1_reshape.jpg", image);
	...
	// debug
	// cout << "type: " << tensor_scores_host->buffer().host << endl;
	// cout << "type: " << tensor_scores_host2->buffer().host << endl;
	// cout << "type: " << tensor_scores_host->host<float>() << endl;
	// cout << "type: " << tensor_scores_host2->host<float>() << endl;

	// cout << "type: " << tensor_scores_host->dimensions() << endl;
	// cout << "type: " << tensor_scores_host2->dimensions() << endl;
	// cout << "type: " << tensor_scores_host->width() << "," << tensor_scores_host->height() << "," << tensor_scores_host->channel() << "," << tensor_scores_host->batch() << "," << endl;
	// cout << "tensor_scores_host2 type: " << tensor_scores_host2->width() << "," << tensor_scores_host2->height() << "," << tensor_scores_host2->channel() << "," << tensor_scores_host2->batch() << "," << endl;
	...
	for (auto box : dest_boxes)
	{
		...
		cv::putText(row_img, std::string(labels[box.label]), pt1, cv::FONT_HERSHEY_DUPLEX, 1, cv::Scalar(0, 255, 0), 2, false);
	}
	// cv::imshow("Yolo-Fastest", row_img);
	// cv::waitKey(0);
	// cv::imshow("Yolo-Fastest", row_img);
	cv::imwrite("../src/1_out.jpg", row_img);
}
