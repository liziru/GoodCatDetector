...
void nms(std::vector<BBox> &input, std::vector<BBox> &output, float iou_threshold, int type)
{
	if (input.size() != 0)
	{
		std::sort(input.begin(), input.end(), [](const BBox &a, const BBox &b)
...
void steering_controller(cv::Point cur_pos, cv::Point prev_pos)
{
	static int i = 10; // 10~170
	static int j = 10; // 10-80
	// static int j_prev = 0; // 10-80
	// static int i_prev = 0; // 10-80

	int size = 10;
	int stride = 6;
	if (cur_pos.x > prev_pos.x + size) // move right
	{
		// i++;
		// i--;
		i -= stride;
	}
	else if (cur_pos.x < prev_pos.x - size)
	{ // move left
		// i--;
		// i++;
		i += stride;
	...
	if (cur_pos.y > prev_pos.y + size) // move down
	{
		// j++;
		// j--;
		j -= stride;
	}
	else if (cur_pos.y < prev_pos.y - size)
	{ // move up
		// j--;
		// j++;
		j += stride;
	...
	if ((j < 80) && (j > 40))
	{
		PCA9685_Set_Rotation_Angle(0, j);
	}
	// j_prev = j;
	// i_prev = i;
...
int main()
{
	...
	const char *labels[20] = {"Aeroplane", "Bicycle", "Bird", "Boat", "Bottle", "Bus", "Car", "Cat", "Chair", "Cow", "Diningtable",
							  "Dog", "Horse", "Motorbike", "Person", "Pottedplant", "Sheep", "Sofa", "Train", "Tvmonitor"};

	string cmd_aplay = "aplay ../../audio/dog_bark.wav";
	int num_apperance = 0;
	...
	PCA9685_Set_Rotation_Angle(0, 40); // 0-top, 80-down
	PCA9685_Set_Rotation_Angle(1, 40); //0-left, 180-right

	// Postion tracker.
	cv::Point cur_pos(0, 0);
	cv::Point center_pos(160, 160);
	...
	while (true)
	{
		...
		cv::Mat image;
		cv::resize(frame, image, cv::Size(INPUT_SIZE, INPUT_SIZE));
		...
		//cout << "dest box size: " << dest_boxes.size() << endl;
		...
		for (auto box : dest_boxes)
		{
			...
			// cv::putText(frame, std::string(labels[box.label]), pt1, cv::FONT_HERSHEY_DUPLEX, 1, cv::Scalar(0, 255, 0), 2, false);
			if (box.label == 7) // find cat.
			{
				if (++num_apperance % 8 == 0)
				{
					cur_pos = (pt2 - pt1) / 2 + pt1;
					// aplay audio
					printf("INFO: %s found! Aplay dog barking.\n", labels[box.label]);
					system(cmd_aplay.c_str());

					// steering engine operations.
					steering_controller(cur_pos, center_pos);
				}
			}
			else
			{
		...
		if (waitKey(70) == 27) // press 'esc' key to exit.
			break;
	}
